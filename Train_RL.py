...
def measure_tour_length_padded(coordinates, tours, lengths):
    ...
    start_coords = coordinates[torch.arange(start=0, end=batch_size, dtype=torch.long), tours.permute(1,0)[0]]
    end_coords = coordinates[torch.arange(start=0, end=batch_size, dtype=torch.long), tours[torch.arange(start=0, end=batch_size, dtype=torch.long), (lengths-1).long()]]
...
def train(gpu, params, run):
    ...
    model = PointerNet(
        ...
        params.hidden, 
        sampling=True,
        glimpse_number=params.glimpse_number, 
        clipping=params.clipping
    )
    # Critic
    critic = CriticNet(params.embedding_size, params.hidden)
    ...
    if params.gpu_number > 0:
        adam = apex.optimizers.FusedAdam
    else:
    ...
    model_scheduler = torch.optim.lr_scheduler.StepLR(model_optim, 2, gamma=0.96, last_epoch=-1, verbose=True)
    critic_scheduler = torch.optim.lr_scheduler.StepLR(critic_optim, 2, gamma=0.96, last_epoch=-1, verbose=True)
    ...
    for epoch in range(params.nof_epoch):
        ...
        for i_batch, sample_batched in enumerate(iterator):
            ...
            if params.AMP:
                with torch.autocast("cuda"):
                    o_model, tour = model(train_batch, sample_batched["sequence_length"][0], max_sequence_length)
                    lengths_model = measure_tour_length_padded(train_batch, tour, sample_batched["sequence_length"][0])
                    lengths_critic = critic(train_batch, sample_batched["sequence_length"][0], max_sequence_length)
                    advantage = (lengths_model - lengths_critic).detach()
                    loss_model = torch.sum(
                        torch.cumsum(
                        advantage.reshape(-1,1) * \
                        torch.log(o_model.permute(1,0,2).gather(-1, tour.reshape(-1,max_sequence_length,1))).reshape(-1,max_sequence_length),
                        -1
                        ).gather(-1, sample_batched["sequence_length"][0].cuda().reshape(-1,1).long()-1).reshape(-1) / sample_batched["sequence_length"][0].cuda())
                    loss_critic = MSE(lengths_model, lengths_critic)
                ...
                loss_model = torch.sum(
                    torch.cumsum(
                    (-1)*advantage.reshape(-1,1) * \
                    torch.log(o_model.permute(1,0,2).gather(-1, tour.reshape(-1,max_sequence_length,1))).reshape(-1,max_sequence_length),
                    -1
                    ).gather(-1, sample_batched["sequence_length"][0].reshape(-1,1).long()-1).reshape(-1) / sample_batched["sequence_length"][0])
                loss_critic = MSE(lengths_model, lengths_critic)
            ...
            if params.AMP:
                ...
                critic_scaler.unscale_(critic_optim)
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0, error_if_nonfinite=False)
                torch.nn.utils.clip_grad_norm_(critic.parameters(), max_norm=1.0, error_if_nonfinite=False)
                scaler.step(model_optim)
                critic_scaler.step(critic_optim)
                scaler.update()
                ...
                torch.nn.utils.clip_grad_norm_(critic.parameters(), max_norm=1.0, error_if_nonfinite=True)
                model_optim.step()
                critic_optim.step()
            ...
            model_losses.append(loss_model.item())
            critic_losses.append(loss_critic.item())
            batch_model_loss.append(loss_model.item())
            batch_critic_loss.append(loss_critic.item())



            iterator.set_postfix(loss='{}, {}'.format(loss_model.item(), loss_critic.item()))
        iterator.set_postfix(loss='{}, {}'.format(np.average(batch_model_loss), np.average(batch_critic_loss)))
        run[f"results_training/average_model_loss"].log(np.average(batch_model_loss))
        run[f"results_training/average_critic_loss"].log(np.average(batch_critic_loss))
        ...
        if summary["averaged"]["average_gap"] < best_gap:
            ...
            torch.save(model, f"checkpoints/ptrnet_rl_model_{tags}_{run_id}_best.pt")
            best_epoch = epoch
            no_improvement_counter = 0
        else:
        ...
        if no_improvement_counter >= params.early_stopping_epoch:
            model = torch.load(f"checkpoints/ptrnet_rl_model_{tags}_{run_id}_best.pt")
            break
    ...
    with open(f"checkpoints/ptrnet_rl_model_{tags}_{run_id}_best_arch.txt", "w") as f: f.write(str(model))
    torch.save(model, f"checkpoints/ptrnet_rl_model_{tags}_{run_id}_best.pt")
    run["model_checkpoints/model_arch"].upload(f"checkpoints/ptrnet_rl_model_{tags}_{run_id}_best_arch.txt")
    run["model_checkpoints/model"].upload(f"checkpoints/ptrnet_rl_model_{tags}_{run_id}_best.pt")
...
def main():
    ...
    parser.add_argument('--glimpse_number', type=int, default=1, help='Number of hidden units')
    parser.add_argument('--clipping', type=float, default=10.0, help='Number of hidden units')
    ...
    if params.tags:
        ...
        params.tags = []
    params.tags.extend(["ptrnet_RL", params.data_source, f"train_low={params.train_low}", f"train_high={params.train_high}", f"train_size={params.train_size}"])
    print(params.tags)
